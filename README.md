# DSH插件市场 · AI 推荐版（dsh-market-ai-recommend）

> 🚀 **本仓库是 [DSH-Plugins-Marketplace](https://github.com/bradeGithub/DSH-Plugins-Marketplace)（v1.5.5）的二次开发分叉（fork）**：保留原版全部能力（一键安装 / 版本检测 / 自动更新 / 已安装识别 / 社区徽章 / 中英双语），新增 **AI 推荐** 体系（每日精选 + 猜你喜欢 + 热门趋势 + 新上架）与 **14 类分类体系**（新增桌面应用 / 音视频）。包名保持 `dsh-plugin-marketplace`，可无缝替换原版安装。

🌐 **语言 / Language:** **中文** | [English](README.en.md)

一个为 [DeepSeek Harness](https://github.com/deepseek-ai/deepseek-harness)（DSH）打造的插件市场插件：自动嗅探 GitHub 生态全部插件，在 DSH Web GUI 设置页中以卡片列表展示，支持**一键安装 / 版本检测 / 自动更新 / 已安装识别**，并内置 **AI 推荐引擎**——「AI 推荐」Tab 每天为你挑选值得一试的插件，全程无需命令行。
...
| | 优势 | 说明 |
|---|---|---|
| 🧠 | **AI 推荐** | 「AI 推荐」Tab：**今日精选**（CI 每天 04:00 UTC 生成，同一天所有人看到同一批）+ **猜你喜欢**（基于你已安装插件的分类/标签相似度）+ **热门趋势** + **新上架**——每条都带规则理由，离线可用 |
...
| 🏷️ | **分类 + 社区徽章** | 构建期自动分类（**14 类**筛选 chips，新增桌面应用 / 音视频）；awesome 聚合页收录的仓库打「社区收录」蓝色徽章 |
...
dsh plugin --profile web install sanniuPUMC/dsh-market-ai-recommend   # 重装即更新
```
...
> 安装 DSH 插件市场插件（dsh-plugin-marketplace）：运行 `dsh plugin --profile web install sanniuPUMC/dsh-market-ai-recommend`；若没有 dsh CLI，则克隆 https://github.com/sanniuPUMC/dsh-market-ai-recommend 到 ~/.dsh/profiles/web/node_modules/dsh-plugin-marketplace，在 ~/.dsh/profiles/web/cordis.patch.yml 中注册（id: plugin-marketplace，name: dsh-plugin-marketplace）。完成后重启 dsh web。

> ⚠️ **从原版迁移**：本 fork 与 [DSH-Plugins-Marketplace](https://github.com/bradeGithub/DSH-Plugins-Marketplace) 使用相同包名 `dsh-plugin-marketplace`——若已装原版，请先 `dsh plugin --profile web remove bradeGithub/DSH-Plugins-Marketplace` 再安装本仓库，避免同包名冲突。
...
4. 点击插件卡片上的按钮：
   ...
   - **更新** → 检测到新版本时覆盖升级
   - **已安装**（灰色）→ 无需操作
5. 切换到 **AI 推荐** tab：**今日精选**（同一天所有人看到同一批，每天自动换批）+ **猜你喜欢**（基于已安装插件的分类/标签相似度）+ **热门趋势** + **新上架**，每条卡片自带推荐理由，可直接一键安装
6. 切换到 **通用 Skills** tab 浏览 20000+ 技能，支持搜索 / 触底分页 / 一键安装

---

## ✨ 功能特性

- **AI 推荐（本 fork 新增）**：「AI 推荐」Tab 聚合四组推荐——
  - **今日精选**：CI 每天 04:00 UTC 从全量索引的质量池（star 门槛 + 社区/验证徽章 + 非玩具仓库过滤）确定性轮换 8 个，覆盖不同分类；前端优先读取 `daily-picks.json`（CDN），不可用/过期时本地规则兜底（同算法、同日结果一致）
  - **猜你喜欢**：基于你已安装插件的**分类 + 标签画像**做内容相似度推荐，理由注明「你已安装同类插件」「与你的常用标签相关」；未安装任何插件时引导先去安装
  - **热门趋势**：log(star) + 近期更新 + 社区收录/人工验证徽章综合打分
  - **新上架**：近 14 天新进入索引的插件按 star 排序
  - 全部推荐**离线可算、零 API 调用**；推荐条目复用完整卡片（含安装/卸载/更新按钮）
...
- **搜索**：按插件名 / 仓库全名 / 标签实时过滤
- **插件分类**：构建时按简介/标签自动分类（**14 类**：视觉多模态 / 文档办公 / 记忆知识 / 模型用量 / 通知通讯 / 开发编码 / 对话会话 / 界面美化 / Agent 自动化 / 通用工具 / 聚合资源 / 桌面应用 / 音视频 / 其他），前端分类 chips 筛选 + 卡片分类徽章（本 fork 新增桌面应用 / 音视频两类，把原「其他」桶中的启动器/托盘/桌宠与音视频/语音/播客类收纳归位）
...
│   │       ├── recommend.js    （AI 推荐引擎：纯函数，猜你喜欢/热门/新上架/每日精选）
...
| `/api/marketplace/recommend` | GET | AI 推荐（本 fork 新增）：`{ date, hasInstalled, guess, trending, fresh, daily, dailySource }`——guess 基于已装插件画像相似度；daily 优先读 CI 生成的 `daily-picks.json`（CDN），`dailySource` 标识 `ci` / `local`，不可用/过期时本地确定性算法兜底；`?refresh=1` 强制刷新 |
...
**分叉致谢（本仓库）**：本仓库是 [DSH-Plugins-Marketplace](https://github.com/bradeGithub/DSH-Plugins-Marketplace)（[bradeGithub](https://github.com/bradeGithub)）的二次开发分叉，AI 推荐引擎（`lib/recommend.js`）、每日精选 CI（`daily-picks.json`）与 14 类分类体系由本仓库新增；原版全部代码与贡献保持 MIT 署名。
...
- 修改推荐引擎：编辑 `lib/recommend.js`（纯函数，服务端与 CI 每日精选共用；改后跑 `node scripts/tests/unit/recommend.test.mjs`）
...
- 修改分类规则：编辑 `scripts/build-registry.mjs` 的 `CATEGORY_RULES` / `CATEGORY_OVERRIDES`（新增分类需同步三处：build-registry 规则 id ↔ `lib/index.js` 的 `CATEGORY_KEYS` ↔ `lib/client.js` 的 `CATEGORY_KEYS` + 中英文案 `cat*` 键，并更新 `audit-expected.json` 期望）
